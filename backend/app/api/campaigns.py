...
def get_video_stats(db: Session = Depends(get_db)):
    ...
    next_publish = db.query(func.min(Video.publish_time)).filter(Video.status.in_([VideoStatus.ready, VideoStatus.pending])).scalar()
    queue_end = db.query(func.max(Video.publish_time)).filter(Video.status.in_([VideoStatus.ready, VideoStatus.pending])).scalar()
...
def get_videos(
    ...
    if status in (VideoStatus.posted.value, VideoStatus.failed.value):
        sort_args = [Video.updated_at.desc(), Video.id.desc()]
    else:
        sort_args = [Video.publish_time.asc(), Video.updated_at.asc(), Video.id.asc()]

    videos = (
        query.order_by(*sort_args)
        .offset(offset)
        .limit(limit)
        .all()
    )
...
def prioritize_video(video_id: str, db: Session = Depends(get_db)):
    ...
    current_status = normalize_status(video.status)
    if current_status not in (VideoStatus.ready.value, VideoStatus.pending.value):
        raise HTTPException(status_code=400, detail="Chỉ có thể ưu tiên video đang ở trạng thái sẵn sàng hoặc chờ tải.")
    ...
    if current_status == VideoStatus.pending.value:
        ...
        try:
            ...
            if out_path:
                ...
                video.last_error = "Tải video thất bại khi đẩy ưu tiên."
                video.retry_count = (video.retry_count or 0) + 1
                db.commit()
                raise HTTPException(status_code=502, detail="Không thể tải video này về máy chủ.")
        except HTTPException:
            raise
        except Exception as exc:
            video.status = VideoStatus.failed
            video.last_error = f"Lỗi tải video: {exc}"
            video.retry_count = (video.retry_count or 0) + 1
            db.commit()
